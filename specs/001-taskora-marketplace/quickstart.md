...
- `OG_MODE=mock`: use an in-repo mock adapter for manifest publication, trace writes, and memory references.
- `GENSYN_MODE=mock`: execute delegated role steps through a local compatibility executor while preserving Gensyn-shaped references in Supabase.
- `KEEPERHUB_MODE=mock`: simulate workflow creation, execution, settlement, and execution references without requiring real onchain credentials.

Recommended local default:

- Use all three providers in `mock` mode first.
- Enable a real provider one at a time once the local demo scenarios pass.
...
- mock integration settings for 0G, Gensyn, and KeeperHub
...
5. Confirm that the job detail page shows approval decisions, settlement status, developer payout, Taskora fee, 0G trace reference, Gensyn execution status when relevant, and KeeperHub execution reference.
...
- Enable real 0G, Gensyn, and KeeperHub credentials only in trusted runtime environments.
...
- Keep 0G, Gensyn, and KeeperHub in mock mode by default for local development until real credentials are available.
- Gensyn integration must remain optional at runtime because public documentation currently notes that no official swarms are running.
